...
- 異常可被視為與常態有顯著偏離的資料  
- 異常偵測是一個識別罕見觀測值的過程，這些觀測值與其所屬的大多數資料有顯著差異 
- 應用範圍包括入侵偵測、詐欺偵測、故障偵測、醫療監控等 

### 信用卡詐欺偵測系統

信用卡詐欺偵測系統（FDS）通常由五個控制層組成。[DP15, DPBC+17],，如圖1所示。
...
前兩層（終端機和交易阻擋規則）是即時執行的（也就是在授權前的幾毫秒內完成）。接下來的兩層（評分規則和資料驅動模型（DDM））則是近即時執行，目的是及時阻擋卡片以防止更多詐欺發生。最後一層（調查人員）是唯一需要人工介入的部分，採離線方式執行。

#### 終端機

終端機是詐欺偵測系統（FDS）的第一層控制機制，負責對所有支付請求執行傳統的安全檢查。安全檢查包括：驗證PIN碼（僅適用於具有晶片的卡片）、嘗試次數、卡片狀態（啟用或停用）、可用餘額以及消費限額。這些操作必須在即時環境下完成（需在幾毫秒內提供回應），期間終端機會向發卡機構的伺服器發出查詢。未通過任何安全檢查的請求會被拒絕，而通過檢查的則成為交易請求，進入第二層控制機制進行處理。

#### 交易阻擋規則

交易阻擋規則是一種「若-則（否則）」的邏輯判斷式，用於阻擋被判定為詐欺的交易請求。這些規則僅使用付款請求時可得的資訊，不會分析歷史紀錄或持卡人資料。舉例來說，阻擋規則可能是：「若為網路交易且為不安全網站，則拒絕交易」。實務上，系統會同時執行多項交易阻擋規則，觸發任一規則的交易都會被阻擋（但不會停用卡片）。交易阻擋規則是由調查人員手動設計，因此屬於詐欺偵測系統中的專家導向元件。為確保即時運作並避免阻擋過多正常交易，阻擋規則必須：一）計算速度快，二）具高度準確性，也就是產生極少的誤報。

所有通過阻擋規則的交易最終都會獲得授權。然而，詐欺偵測活動會在交易資料加入彙總特徵後持續進行，這些特徵用於將當前交易與過往交易及持卡人資料進行情境化比對。彙總特徵包括：平均消費金額、同一天的平均交易次數，或先前購物地點等。這個計算彙總特徵的過程稱為「特徵工程」或「特徵擴增」。擴增特徵與當前交易資料會整合成一個「特徵向量」，用於判斷已授權的交易是否為詐欺或正常交易。詐欺偵測系統的「評分規則」和「資料驅動模型（DDM）」層就是根據這個特徵向量進行運作。

#### 評分規則

評分規則也是專家導向模型，以「若-則（否則）」*if-then（-else）*邏輯判斷式表示。這些規則運用特徵向量，為每筆已授權的交易指派分數：分數越高，代表交易越可能是詐欺。評分規則由調查人員手動設計，並由其自行定義相關分數。舉例來說，一個評分規則可能是：「若上一筆交易發生在不同洲且與上一筆交易時間間隔少於一小時，則詐欺分數 = 0.95」。然而，評分規則僅能偵測調查人員已發現的詐欺手法，且這些手法僅涉及特徵向量中的少數元件。此外，評分規則較為主觀，因為不同專家會設計不同規則。最後，這些規則可能不夠完整，且難以隨時間維護。

#### 資料驅動模型（DDM）

此層純粹以資料驅動為基礎，採用分類器或其他統計模型來評估每個特徵向量成為詐欺的機率。這個機率會作為已授權交易的詐欺分數。因此，資料驅動模型是透過已標記的交易資料集進行訓練，且無法被調查人員解釋或手動修改。一個有效的資料驅動模型預期能夠透過同時分析特徵向量的多個元件來偵測詐欺模式，可能涉及非線性表達式。因此，資料驅動模型有望發現超越調查人員經驗的詐欺案例，且這些規則不一定能被輕易解釋。

#### 調查人員

調查人員是分析信用卡交易的專業人士，負責詐欺偵測系統中的專家導向層面。特別是，調查人員負責設計交易阻擋規則和評分規則。

調查人員也負責控管由評分規則和資料驅動模型（DDM）所產生的警示，判斷這些警示是真實詐欺還是誤報 [[DP15](https://fraud-detection-handbook.github.io/fraud-detection-handbook/Chapter_References/bibliography.html#id34)]。具體來說，他們透過案件管理工具檢視所有可疑交易，該工具會呈現交易的所有資訊，包括指派的分數/機率值，這些數值實際上代表每筆交易的*風險程度*。調查人員會與持卡人聯繫確認後，將警示交易標記為「正常」或「詐欺」，並將這些資訊回饋給詐欺偵測系統。在後續討論中，我們將這些已標記的交易稱為回饋資料，並使用警示-回饋互動機制來描述這個在實際詐欺偵測系統中產生監督式資訊的過程。

任何疑似遭詐欺的卡片都會立即被停用，以防止進一步的詐欺行為。通常，調查人員會檢查遭盜用卡片的所有近期交易，這意味著每個被偵測到的詐欺案件可能會產生多筆回饋資料，且這些資料不一定都對應到警示或詐欺。在實際的詐欺偵測系統中，由於這個過程既耗時又繁瑣，調查人員每天只能檢查有限的警示數量 [[Car18](https://fraud-detection-handbook.github.io/fraud-detection-handbook/Chapter_References/bibliography.html#id31), [DP15](https://fraud-detection-handbook.github.io/fraud-detection-handbook/Chapter_References/bibliography.html#id34), [Kri10](https://fraud-detection-handbook.github.io/fraud-detection-handbook/Chapter_References/bibliography.html#id45)]。因此，資料驅動模型的首要目標是提供精確的警示，因為當系統產生過多誤報時，調查人員可能會忽略後續的警示。

在最新的系統中，具有極高風險分數的交易可以跳過調查人員，直接透過簡訊等方式向持卡人要求確認。這項功能可加快處理速度並擴大回饋範圍，但必須謹慎使用，以免損害客戶信任。

### 總結

信用卡詐欺偵測（CCFD）是一項極具挑戰性的任務，需要分析大量交易資料以識別詐欺模式。由於資料量龐大且詐騙手法不斷推陳出新，單靠人工調查已無法有效因應這個問題。過去十年間，CCFD 領域逐漸導入機器學習（Machine Learning，ML）演算法，這項技術能夠從海量資料中搜尋和偵測出特定模式。實證顯示，ML 演算法確實能顯著提升詐欺偵測系統的效能，協助詐欺調查人員識別可疑交易。

ML 在 CCFD 的應用已成為活躍的研究領域，從大量相關文獻及其展現的應用潛力可見一斑。然而，在解讀這些研究成果時仍需謹慎。如同先前所提及，ML 在 CCFD 的應用仍屬新興領域，目前尚未建立公認的基準或評估方法來比較與評估各種技術的優劣。

本書旨在為此邁出第一步，提供可重現的實作方案和基礎方法論，以探討 ML 在 CCFD 的應用。這些建議的實作方案和方法論建基於我們在這個領域的實務經驗，來自[布魯塞爾大學機器學習研究組](https://mlg.ulb.ac.be/wordpress/)與[Worldline支付處理公司](https://uk.worldline.com/en/home.html)長達十年的合作成果。
...
![How to detect fraud](https://seon.io/wp-content/uploads/2023/09/5.1-Credit-Card-Fraud-How-to-Detect.png "How to detect fraud")
